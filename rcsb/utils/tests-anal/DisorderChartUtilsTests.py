...
###
    def testViewGapData(self):
        ...
        try:
            ...
            cu.doIntegerBarChart(gapCountList, plotPath=self.__plotGapCount, yPlotScale='log',
                                 yPlotMax=6, xPlotMax=30, xPlotLabel="Gap Count", yPlotLabel="Protein Instances (log)",
                                 plotTitle="Protein Instance Gap Count")
            self.__writeLegend(
                self.__plotGapCount,
                "Gap count statistics for all (%d) protein sequences (%d X-ray structures with resolution limit < 3.5 Angstoms) " %
                (len(entryCountD), len(gapCountList)))
            ...
            self.__writeLegend(
                ...
                "Gap width statistics for all (%d) protein sequences (%d X-ray structures with resolution limit < 3.5 Angstoms) " %
                (len(entryCountD), len(gapLengthList)))
        except Exception as e:
            logger.exception("Failing with %s" % str(e))
            self.fail()
    ...
    def testViewCoverageData(self):
        ...
        try:
            ...
            self.__writeLegend(
                ...
                "Sample sequence coverage for all (%d) protein sequences (%d X-ray structures with resolution limit < 3.5 Angstoms) " %
                (len(covSampleList), len(entryCountD)))
            #
    ...
    def testTaxaData(self):
        ...
        try:
            ...
            for entryId in entryD:
                ...
                # if iCount > 5000:
                #    break
            logger.info("Eukaryota %d Bacteria %d Virus %d Archaea %d Other/Syn %r Unclas %d" % (numEukaryota, numBacteria, numVirus, numArchaea, numOther, numUnclass))
        except Exception as e:
            logger.exception("Failing with %s" % str(e))
            self.fail()
